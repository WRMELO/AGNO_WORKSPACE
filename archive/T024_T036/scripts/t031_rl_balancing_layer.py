...
OUT_LEDGER = Path("src/data_engine/portfolio/SSOT_PORTFOLIO_LEDGER.parquet")
OUT_CURVE = Path("src/data_engine/portfolio/SSOT_PORTFOLIO_CURVE.parquet")
...
def main() -> None:
    ...
    prices = prices.dropna(subset=["ticker", "date", "close_operational"])
    ...
    standings = standings.dropna(subset=["ticker", "date", "standings_rank", "f1_score_total"])
    ...
    for idx, d in enumerate(sim_dates):
        ...
        # Policy Engine: target exposure 0..1
        target = 1.0
        ...
        # 3) Rebalance up: compra top F1 se abaixo do target
        buy_allowed = (idx % BUY_HEARTBEAT_DAYS == 0) and (not market_panic)
        if buy_allowed and current_exposure < target - BUFFER and not day_stand.empty:
            top = day_stand.sort_values(["standings_rank", "ticker"]).head(TOP_N)
    ...
    if ledger.empty:
        ledger = pd.DataFrame(
            columns=["date", "ticker", "side", "reason", "qty", "price", "notional", "net_notional", "cost_brl", "cash_before", "cash_after", "target_exposure"]
        )
    else:
    ...
    print("TASK T031 - RL BALANCING LAYER")
    print(f"DATES_SIMULATED: {len(curve)}")
    print(f"TRADES: {len(ledger)}")
    print(f"AVG_EXPOSURE: {curve['current_exposure'].mean():.4f}")
    print(f"MIN_EXPOSURE: {curve['current_exposure'].min():.4f}")
    print(f"MAX_EXPOSURE: {curve['current_exposure'].max():.4f}")
    print(f"PANIC_BUYS: {int(ledger[(ledger['side']=='BUY') & (ledger['date'].isin(curve[curve['market_panic']]['date']))].shape[0])}")
