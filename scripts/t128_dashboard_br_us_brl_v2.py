...
TASK_ID = "T128"
RUN_ID = "T128-BR-US-BRL-DASHBOARD-V2"
ROOT = Path("/home/wilson/AGNO_WORKSPACE")
PYTHON_ENV = ROOT / ".venv/bin/python"
CHANGELOG_PATH = ROOT / "00_Strategy/changelog.md"

IN_BR_WINNER = ROOT / "src/data_engine/portfolio/CTO_C060_EXPANDED_ABLATION_WINNER_CURVE.parquet"
...
OUT_SCRIPT = ROOT / "scripts/t128_dashboard_br_us_brl_v2.py"
OUT_PLOT = ROOT / "outputs/plots/T128_STATE3_PHASE10E_BR_US_BRL_DASHBOARD_V2.html"
OUT_REPORT = ROOT / "outputs/governanca/T128-BR-US-BRL-DASHBOARD-V2_report.md"
OUT_MANIFEST = ROOT / "outputs/governanca/T128-BR-US-BRL-DASHBOARD-V2_manifest.json"
OUT_EVID = ROOT / "outputs/governanca/T128-BR-US-BRL-DASHBOARD-V2_evidence"
...
TRACEABILITY_LINE = "- 2026-03-04T21:28:03Z | EXEC: T128 PASS. Dashboard BR+US em BRL (V2: BR winner=C060X canônico; compat schema equity_end_norm/ret_cdi; visual apenas, sem split). Artefatos: scripts/t128_dashboard_br_us_brl_v2.py; outputs/plots/T128_STATE3_PHASE10E_BR_US_BRL_DASHBOARD_V2.html; outputs/governanca/T128-BR-US-BRL-DASHBOARD-V2_{report,manifest}.json"
...
def main() -> int:
    ...
    try:
        ...
        if not inputs_present:
            raise RuntimeError("required inputs missing for T128-V2")
        ...
        schema_ok = {"date", "split", "equity_end_norm", "ret_cdi"}.issubset(set(br.columns))
        ...
        gates.append(Gate("G_INPUT_SCHEMA_OK", schema_ok, "BR(C060X)/US/FX required columns"))
        if not schema_ok:
        ...
        br["equity_br_brl"] = pd.to_numeric(br["equity_end_norm"], errors="coerce")
        br["ret_cdi"] = pd.to_numeric(br["ret_cdi"], errors="coerce").fillna(0.0)
        br["equity_cdi_brl"] = INITIAL_CAPITAL * (1.0 + br["ret_cdi"]).cumprod()
        ...
        fig = make_subplots(
            ...
            cols=2,
            subplot_titles=(
                "Equity Normalizada em BRL (BR Winner C060X vs US Winner convertido vs SP500 convertido vs CDI)",
        ...
        fig.add_trace(go.Scatter(x=panel["date"], y=panel["norm_equity_br_brl"], mode="lines", name="BR Winner C060X (BRL)"), row=1, col=1)
        fig.add_trace(go.Scatter(x=panel["date"], y=panel["norm_equity_us_brl"], mode="lines", name="US Winner T122 em BRL"), row=1, col=1)
        ...
        fig.add_trace(go.Scatter(x=panel["date"], y=panel["norm_equity_br_brl"], mode="lines", name="BR Winner C060X BRL", showlegend=False), row=1, col=2)
        fig.add_trace(go.Scatter(x=panel["date"], y=panel["norm_equity_us_usd"], mode="lines", name="US Winner T122 USD", showlegend=False), row=1, col=2)

        fig.add_trace(go.Scatter(x=panel["date"], y=panel["dd_equity_br_brl"], mode="lines", name="DD BR Winner C060X", showlegend=False), row=2, col=1)
        ...
        fig.add_annotation(
            ...
            y=1.11,
            showarrow=False,
            text="Fábricas independentes, capital próprio, sem split. BR winner canônico: C060X. Painel é somente visual/financeiro em BRL.",
        )
