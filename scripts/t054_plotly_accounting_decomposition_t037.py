#!/usr/bin/env python3
"""T054 - Plotly accounting decomposition (P&L/Cost/Cash) for T037 baseline."""
...
TASK_ID = "T054-ACCOUNTING-PLOTLY-T037-V1"
ROOT = Path("/home/wilson/AGNO_WORKSPACE")

INPUT_CURVE = ROOT / "src/data_engine/portfolio/T037_PORTFOLIO_CURVE.parquet"
INPUT_LEDGER = ROOT / "src/data_engine/portfolio/T037_PORTFOLIO_LEDGER.parquet"
INPUT_TASK_SPEC = (
    ROOT
...
OUTPUT_HTML = ROOT / "outputs/plots/T054_T037_ACCOUNTING_DECOMPOSITION.html"
OUTPUT_REPORT = ROOT / "outputs/governanca/T054-ACCOUNTING-PLOTLY-T037-V1_report.md"
OUTPUT_MANIFEST = ROOT / "outputs/governanca/T054-ACCOUNTING-PLOTLY-T037-V1_manifest.json"
OUTPUT_EVIDENCE_DIR = ROOT / "outputs/governanca/T054-ACCOUNTING-PLOTLY-T037-V1_evidence"
...
SCRIPT_PATH = ROOT / "scripts/t054_plotly_accounting_decomposition_t037.py"
...
def main() -> int:
    ...
    # T037 pode não conter a coluna de regime defensivo; fallback para sem sombreamento.
    regime = daily.get("regime_defensivo", pd.Series(False, index=daily.index)).fillna(False).astype(bool)
    for x0, x1 in find_true_intervals(regime, daily["date"]):
        fig.add_vrect(x0=x0, x1=x1, fillcolor="rgba(255,0,0,0.06)", line_width=0)

    fig.update_layout(
        title="T054 T037 - Accounting Decomposition (P&L/Cost/Cash)",
    ...
    report_lines = [
        ...
        "## 3) Observations",
        "",
        "- `cdi_credit` foi exibido e auditado exatamente como materializado no `T037_PORTFOLIO_CURVE.parquet` (sem reinterpretação).",
