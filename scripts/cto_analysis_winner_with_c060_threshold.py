...
"""CTO exploratory analysis: Winner Phase 8 params (top_n=15) + C060 threshold on expanded universe.

Tests the effect of "unlocking" the engine — more time in market with 15 positions
over the expanded universe (BR+BDR+US) using C060 hysteresis (thr=0.25, h_in=3, h_out=2).

Parameters:
  top_n=15, threshold=0.25, h_in=3, h_out=2
  cadence_mode_a=3, cadence_mode_b=10
  target_pct=1/15≈0.0667, max_pct=0.10
  cost=2.5bps uniform
"""
...
OUT_CURVE = ROOT / "src/data_engine/portfolio/CTO_WINNER_C060_THRESHOLD_CURVE.parquet"
...
def main() -> int:
    print("=" * 70)
    print("CTO ANALYSIS: Winner N=15 + C060 threshold on expanded universe")
    print(f"  top_n={TOP_N}, thr={THR}, h_in={H_IN}, h_out={H_OUT}")
    print(f"  target_pct={TARGET_PCT:.4f}, max_pct={MAX_PCT:.2f}")
    print("=" * 70)
    ...
    print("[4/7] Configuring N=15 + C060-threshold params on T072 engine...")
    t072_cfg = json.loads(INPUT_T072_CFG.read_text(encoding="utf-8"))

    t072.PROFILE_A["top_n"] = TOP_N
    t072.PROFILE_A["target_pct"] = TARGET_PCT
    t072.PROFILE_A["max_pct"] = MAX_PCT
    t072.PROFILE_B["top_n"] = TOP_N
    t072.PROFILE_B["target_pct"] = TARGET_PCT
    t072.PROFILE_B["max_pct"] = MAX_PCT
    ...
    print(f"  N=15 params: top_n={TOP_N}, thr={THR}, h_in={H_IN}, h_out={H_OUT}")
    print(f"  target_pct={TARGET_PCT:.4f}, max_pct={MAX_PCT}")
    ...
    print("[5/7] Running dual-mode backtest on expanded universe with N=15...")
    ...
    print("[6/7] Applying ML trigger (C060 hysteresis thr=0.25) on N=15 base curve...")
    ...
    out_ml["curve_name"] = "CTO_WINNER_C060_THRESHOLD"
    ...
    def report_metrics(label: str, curve: pd.DataFrame) -> dict[str, Any]:
        ...
        for split_name, sub in [
            ...
            cash_days = float(sub["state_cash"].sum()) if "state_cash" in sub.columns else 0.0
            total_days = float(len(sub))
            results[split_name] = {**m, "switches": sw, "time_in_cash": cash_days / total_days if total_days > 0 else 0.0}
        print(f"\n  {label}:")
        for k, v in results.items():
            print(f"    {k}: equity={v['equity_final']:,.2f}  CAGR={v['cagr']*100:.2f}%  MDD={v['mdd']*100:.2f}%  Sharpe={v['sharpe']:.3f}  switches={v['switches']:.0f}  cash%={v['time_in_cash']*100:.1f}%")
        return results

    print("\n" + "=" * 70)
    print("METRICS — N=15 Expanded + C060 Threshold")
    print("=" * 70)
    report_metrics("N15 Expanded thr=0.25 (BR+BDR+US)", out_ml)
